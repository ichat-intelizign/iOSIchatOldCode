...
extension Message: ModelMappeable {
    ...
    func map(_ values: JSON, realm: Realm?) {
        ...
        if let newname = values["u"]["name"].string {
        ...
        if let actionLink = values["actionLinks"].array {
            self.actionL.removeAll()
            for attachment1 in actionLink {
                let obj = Actionlink()
                obj.map(attachment1, realm: realm)
                self.actionL.append(obj)
            }
        }
