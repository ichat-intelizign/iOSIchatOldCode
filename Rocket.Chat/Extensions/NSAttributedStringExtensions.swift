...
extension NSMutableAttributedString {
    ...
    func highlightMentions(for message: Message) {
        message.mentions.forEach {
            if var username = $0.username {

//                let auth: Auth? = AuthManager.isAuthenticated()
//                let newsubscription = auth?.subscriptions.filter("name = %@", username).first
//                username = (newsubscription?.fname)!
...
//    }
    ...
    func setMention(_ mention: String, range: NSRange? = nil) {
        let link = "ichat.intelizi.com://mention?name=\(mention)"
    ...
    func setlinkontext(for message: Message)
    {
        message.mentions.forEach {
            if let username = $0.username {
                let attributedString = NSMutableAttributedString(string: username)
                let linkWasSet = attributedString.setAsLink(textToFind: username, linkURL: "http://stackoverflow.com")

                if linkWasSet {
                    print("linkwaset\(linkWasSet)")
                }
            }
        }
    }
