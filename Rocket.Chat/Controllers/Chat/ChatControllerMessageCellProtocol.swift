...
extension ChatViewController: ChatMessageCellProtocol, MyChatCellProtocol {
...
//            controller?.subscription = directMessageRoom
            let loginVC = UIStoryboard(name: "Chat", bundle: nil).instantiateViewController(withIdentifier: "ChannelInfoViewController") as? ChannelInfoViewController
            loginVC?.frommemberlist = "member"
            loginVC?.nameofuser = username
    ...
    func openImageFromCell(attachment: Attachment, thumbnail: FLAnimatedImageView) {
        ...
        if thumbnail.animatedImage != nil || thumbnail.image != nil {
            let configuration = ImageViewerConfiguration { config in
               ...
               // config.imageUrls = attachment.value(forKey: "imageURL") as? String
            }
