...
extension MessagesListViewController: UICollectionViewDataSource {
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        if cellData.message?.user?.username != AuthManager.currentUser()?.username {
            if let message = cellData.message,
                ...
                bubble_width = max(cell.messageContainerView.frame.origin.x + cell.messageContainerView.frame.size.width + marginLeft, cell.messageContainerView.frame.origin.x + cell.messageContainerView.frame.size.width + 2 * marginLeft)
                return cell
            }
        }
        ...
        if cellData.message?.user?.username == AuthManager.currentUser()?.username {
            if let message = cellData.message,
                let cell = collectionView.dequeueReusableCell(withReuseIdentifier: MyChatCell.identifier, for: indexPath) as? MyChatCell {
                cell.delegate = ChatViewController.shared
                cell.message = message
                let marginLeft: CGFloat = 5
                let marginRight: CGFloat = 2
                //Bubble positions
                var bubble_x: CGFloat = 0
                let bubble_y: CGFloat = 0
                var bubble_width: CGFloat
                bubble_x = min(cell.messageContainerView.frame.origin.x - marginLeft, cell.messageContainerView.frame.origin.x - 2 * marginLeft)
                print("bubble_x -\(bubble_x)")
                 cell.bubbleImage.image = UIImage(named: "bubbleMine")?.stretchableImage(withLeftCapWidth: 8, topCapHeight: 30)
                bubble_width = cell.contentView.frame.size.width - bubble_x - marginRight
                //  let bubble_height: CGFloat = cell.messageContainerView.frame.size.height
                //  cell.bubbleImage.frame = CGRect(x: bubble_x, y: bubble_y, width: bubble_width, height: bubble_height)
                //cell.bubbleImage.autoresizingMask = cell.messageContainerView.autoresizingMask
...
extension MessagesListViewController: UICollectionViewDelegateFlowLayout {
    func collectionView(_ collectionView: UICollectionView, layout collectionViewLayout: UICollectionViewLayout, sizeForItemAt indexPath: IndexPath) -> CGSize {
        ...
        if cellData.message?.user?.username == AuthManager.currentUser()?.username {
            if let message = cellData.message {
                return CGSize(width: fullWidth, height: MyChatCell.cellMediaHeightFor(message: message, sequential: false) + 20)
            }
        }
        if cellData.message?.user?.username != AuthManager.currentUser()?.username {
            if let message = cellData.message {
                return CGSize(width: fullWidth, height: ChatMessageCell.cellMediaHeightFor(message: message, sequential: false) + 20)
            }
        }
