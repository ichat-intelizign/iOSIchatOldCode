...
final class ChatMessageTextView: UIView {
    ...
    private func updateLabels() {
       let orignal = viewModel?.title.chopPrefix()
        let auth: Auth? = AuthManager.isAuthenticated()
        let newsubscription = auth?.subscriptions.filter("name = %@", orignal ?? "").first
        print("newtypingsssubscription - \(newsubscription?.fname)")
