...
extension SubscriptionManager {
    static func subscribeTypingEvent(_ subscription: Subscription, completion: @escaping (String?, Bool) -> Void) {
        ...
        SocketManager.subscribe(request, eventName: eventName) { response in
            ...
            if let id = userNameTyping {
                let auth: Auth? = AuthManager.isAuthenticated()
                let newsubscription = auth?.subscriptions.filter("name = %@", id).first
                print("newtypingsssubscription - \(newsubscription)")
                let flag = (msg[1].int ?? 0) > 0

                completion(newsubscription?.fname, flag)
            }
...
//    static func getUserInfo(byName name: String, completion: @escaping MessageCompletion) {
//        let request = [
//            "msg": "method",
//            "method": "avatar",
//            "params": ["c", name]
